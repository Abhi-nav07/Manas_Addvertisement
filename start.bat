...
set "APP_DIR=."
if exist "manas-advertising\package.json" (
    set "APP_DIR=manas-advertising"
)

:: If backend exists, start backend in a separate terminal window
if exist "manas-backend\package.json" (
    echo [INFO] Starting Backend Service in background...
    start "Manas Advertising Backend" cmd /k "cd /d manas-backend && title Manas Backend && echo [INFO] Checking Backend dependencies... && if not exist node_modules (echo [INFO] Installing backend dependencies... && npm install) && echo [INFO] Starting Backend server... && npm run dev"
)
...
if not exist "node_modules\" (
    echo [INFO] Dependencies - node_modules - missing in %APP_DIR%.
